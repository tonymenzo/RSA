...
repeat = {100:100, 500: 50, 1000:20, 2500:10, 5000:10, 7500:10, 10000:10, 25000:5, 50000:3}
...
for N_events in Ns:
    ...
    for r in range(0,repeat[N_events]):
        ...
        sim_mult = np.array([len(sim_hadrons[i,:][np.abs(sim_hadrons[i,:,0]) > 0.0]) for i in range(len(exp_hadrons))])
        ...
        all_gradients_t.append(a_b_gradients)
        all_loss_grid_t.append(loss_grid)
        all_metrics_mu_t.append(metrics[0])
        all_metrics_Neff_t.append(metrics[1])
    all_gradients.append(all_gradients_t)
    all_loss_grid.append(all_loss_grid_t)
    all_metrics_mu.append(all_metrics_mu_t)
    all_metrics_Neff.append(all_metrics_Neff_t)
...
np.save(f'/pscratch/sd/l/ljpuslar/RSA/RSA/src/temp_results/grad_stability_errors/all_loss_grid{plt_nm}',
        np.array(all_loss_grid, dtype=object))
