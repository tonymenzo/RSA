...
def main():
    ...
    for it in range(Nt):
        random_indices_target = np.random.choice(N, size=N_events, replace=False)
        for ib in range(Nb):

            # Convert into torch objects
            print(sim_accept_reject.shape)
            sim_accept_reject_t = torch.Tensor(sim_accept_reject[random_indices_target])
            sim_fPrel_t         = torch.Tensor(sim_fPrel[random_indices_target])

            # sim_scores          = torch.Tensor(sim_observable[:, random_indices].copy())
            # exp_scores          = torch.Tensor(exp_observable[:, random_indices].copy())

            sim_scores          = [x[random_indices_target] for x in sim_observable]
            exp_scores          = [x[random_indices_target] for x in exp_observable]
