...
# Add .../RSA/RSA/src to PYTHONPATH dynamically
THIS_FILE = Path(__file__).resolve()
SRC_DIR = THIS_FILE.parents[1]          # .../RSA/RSA/src
# If your script is in src/Coverage_tests/, parents[1] is src/
...
# Add argument parsing for distributed runs
def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--nt-total", type=int, default=25)
    p.add_argument("--nb", type=int, default=25)
    p.add_argument("--nt-chunk", type=int, default=5)
...
def main():
    ...
    if it_start >= NT_total:
        raise ValueError(f"chunk-id {chunk_id} starts at {it_start} >= NT_total {NT_total}")

    NT_local = it_end - it_start
    ...
    N_target_draw = 100_000   # number of target events per pseudo-experiment
    ...
    # allocate local arrays only
    D = len(group_order)
    theta_hat = np.zeros((NT_local, NB, D))
    mu_t = np.zeros((NT_local, D))
    ...
    for it_local in range(NT_local):
        ...
        mu_t[it_local, :] = theta_hat[it_local, :, :].mean(axis=0)
        sig_t[it_local, :] = theta_hat[it_local, :, :].std(axis=0, ddof=1)

        # CI and coverage (per dimension)
        lo = mu_t[it_local, :] - k_sigma * sig_t[it_local, :]
        hi = mu_t[it_local, :] + k_sigma * sig_t[it_local, :]
        cov_t[it_local, :] = ((theta_true >= lo) & (theta_true <= hi)).astype(np.int32)

        print(f"chunk {chunk_id}: finished it_global={it_global}")
        print(f"--- Target bootstrap it={it_local+1}: mu={mu_t[it_local,:]}, sigma={sig_t[it_local,:]}, cov={cov_t[it_local,:]}")
        ...
        tag = f"NT{NT_total}_NB{NB}_chunk{chunk_id}_it{it_start}-{it_end-1}"

        np.save(out / f"theta_hat_{tag}.npy", theta_hat)
        np.save(out / f"mu_t_{tag}.npy", mu_t)
    ...
    if it_global + 1 >= NT_total:  # only save summary after the last chunk finishes
        with open(os.path.join(out, f"summary_NT{NT_total}_NB{NB}_k{k_sigma}.txt"), "w") as f:
            f.write(f"NT={NT_total}\nNB={NB}\nk_sigma={k_sigma}\n")
