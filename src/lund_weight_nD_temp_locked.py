...
# lock parameters a_d=a_u=a_s=... and b_d=b_u=b_s=... 
...
class LundWeight(nn.Module):
    def __init__(self, params_base, params, over_sample_factor, device = 'cuda', locked=False):
        ...
        #     for key in params_base.keys():
        #         if 'a' in key and at:
        #             a_param = torch.nn.Parameter(params[key].to(self.device), requires_grad = True)
        #             at = False
        #         elif 'b' in key and bt:
        #             b_param = torch.nn.Parameter(params[key].to(self.device), requires_grad = True)
        #             bt = False

        #         if key in params:
        #             if 'a' in key:
        #                 self.params[key] = a_param
        #             elif 'b' in key:
        #                 self.params[key] = b_param
        #             else:
        #                 self.params[key] = torch.nn.Parameter(params[key].to(self.device), requires_grad = True)
        #         else:
        #             self.params[key] = torch.nn.Parameter(params_base[key].clone().detach().to(self.device), requires_grad = False)

        # Locked sharing: make all a_* share the first a_* param; same for b_*
        if locked:
            a_param = None
            b_param = None

            # Choose deterministic "first" by sorting (optional; remove sorted(...) to keep original order)
            for key in sorted(params_base.keys()):
                if key in params and key.startswith('a'):
                    if a_param is None:
                        # Reuse the already-registered object; do NOT re-wrap
                        # a_param = self.params[key]
                        a_param = torch.nn.Parameter(params[key].to(self.device), requires_grad = True)
                        # (Optionally ensure it's trainable)
                        # if not a_param.requires_grad:
                        #     a_param.requires_grad_(True)
                    self.params[key] = a_param  # share the same object
                elif key in params and key.startswith('b'):
                    if b_param is None:
                        b_param = torch.nn.Parameter(params[key].to(self.device), requires_grad = True)
                        if not b_param.requires_grad:
                ...
                # else:
                #   - keys not in params stay frozen (as set above)
                #   - non a_/b_ keys in params remain their own learnable params
