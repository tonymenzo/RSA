...
int main() {
  ...
  // Set data PATHs
  string hadron_PATH       = "pgun_data/model_1/pgun_uubar_hadrons_target_N_1e1.txt";
  string acceptReject_PATH = "pgun_data/model_1/pgun_uubar_accept_reject_z_target_N_1e1.txt";
  string mT2_PATH          = "pgun_data/model_1/pgun_uubar_mT2_target_N_1e1.txt";
  string fPrel_PATH        = "pgun_data/model_1/pgun_uubar_fPrel_target_N_1e1.txt";
  string id_PATH           = "pgun_data/model_1/pgun_uubar_pid_target_N_1e1.txt";
  ...
  // Set flavor-dependent fragmentation parameters following RSA-mod.

  // d-quark.
  pythia.readString("StringZ:aExtraDQuark = -0.07");
  pythia.readString("StringZ:useNonstandardD = true");
  pythia.readString("StringZ:aNonstandardD = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardD = 1.0");
  pythia.readString("StringZ:rFactD = 0.0"); // No Bowler modification
  // u-quark.
  pythia.readString("StringZ:aExtraUQuark = 0.1"); 
  pythia.readString("StringZ:useNonstandardU = true");
  pythia.readString("StringZ:aNonstandardU = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardU = 0.88");
  pythia.readString("StringZ:rFactD = 0.0"); // No Bowler modification
  // s-quark.
  pythia.readString("StringZ:aExtraSQuark = -0.2"); // 0.0 is default
  pythia.readString("StringZ:useNonstandardS = true");
  pythia.readString("StringZ:aNonstandardS = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardS = 1.0");
  pythia.readString("StringZ:rFactS = 0.0"); // No Bowler modification
  // c-quark.
  pythia.readString("StringZ:aExtraCquark = 0.3");
  pythia.readString("StringZ:useNonstandardC = true");
  pythia.readString("StringZ:aNonstandardC = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardC = 0.98");
  pythia.readString("StringZ:rFactC = 0.0"); // No Bowler modification
  // b-quark.
  pythia.readString("StringZ:aExtraBquark = 0.0");
  pythia.readString("StringZ:useNonstandardB = true");
  pythia.readString("StringZ:aNonstandardB = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardB = 1.3");
  pythia.readString("StringZ:rFactB = 0.0"); // No Bowler modification
  // di-quark.
  pythia.readString("StringZ:aExtraDiquark = 0.97"); // 0.97 is default
  pythia.readString("StringZ:useNonstandardH = true");
  pythia.readString("StringZ:aNonstandardH = 0.68"); // Monash
  pythia.readString("StringZ:bNonstandardH = 1.2");
  pythia.readString("StringZ:rFactH = 0.0"); // No Bowler modification
  ...
  pythia.readString("StringFlav:popcornSpair = 0");
  pythia.readString("StringFlav:popcornSmeson = 0");
  ...
  ofstream cfragfileStart("pgun_data/acceptReject_i.txt", ios::out | ios::trunc);
  cfragfileStart.close();

  // Clear the temporary mT2 file
  ofstream cMT2fileStart("pgun_data/mT2_i.txt", ios::out | ios::trunc);
  cMT2fileStart.close();

  // Clear the temporary fPrel file
  ofstream cfPrelfileStart("pgun_data/fPrel_i.txt", ios::out | ios::trunc);
  cfPrelfileStart.close();

  // Clear the temporary id file
  ofstream cidfileStart("pgun_data/id_i.txt", ios::out | ios::trunc);
  cidfileStart.close();

  // Clear the temporary hadron file
  ofstream chadronfile(hadron_PATH, ios::out | ios::trunc);
  chadronfile.close();

  // Clear the temporary accept-reject file
  ofstream cacceptRejectfile(acceptReject_PATH, ios::out | ios::trunc);
  cacceptRejectfile.close();

  // Clear the temporary mT2 file
  ofstream cmT2file(mT2_PATH, ios::out | ios::trunc);
  cmT2file.close();

  // Clear the temporary fPrel file
  ofstream cfPrelHadronfile(fPrel_PATH, ios::out | ios::trunc);
  cfPrelHadronfile.close();

  // Clear the temporary id file
  ofstream cidHadronfile(id_PATH, ios::out | ios::trunc);
  cidHadronfile.close();
  ...
  do {
    ...
    ifstream rfile("pgun_data/acceptReject_i.txt", ios::in | ios::out);
    ifstream mT2file("pgun_data/mT2_i.txt", ios::in | ios::out);
    ifstream fPrelfile("pgun_data/fPrel_i.txt", ios::in | ios::out);
    ifstream idfile("pgun_data/id_i.txt", ios::in | ios::out);
    string line;
    int nLines = 0;
    ...
    if (nLines == nHadrons) {
      ...
      // Create id file.
      ofstream iddatafile(id_PATH, ios::out | ios::in | ios::app);
      // Copy the event into the data file.
      while (getline(idfile, line)) {
        iddatafile << line << endl;
      }
      // Input a new line to separate events
      iddatafile << endl;
      iddatafile.close();
      ...
      // Write out fPrel for accepted and rejected chains with some convention denoting the rejected chains.
      lineCounter = 0;
      // Append to the file
      ofstream fPreldatafile(fPrel_PATH, ios::out | ios::in | ios::app);
      // Append to file while the line counter is less than the difference between the number of lines and number of hadrons (iterate until we hit the accepted chain)
      ...
      // Write out fPrel for accepted and rejected chains, separating the accepted and rejected chains by the string '&'
      fPreldatafile << "&" << endl;
      // Append the rejected fragmentations
      while (getline(fPrelfile, line)) {
        fPreldatafile << line << endl;
      ...
      // Write out id for accepted and rejected chains with some convention denoting the rejected chains.
      lineCounter = 0;
      // Append to the file
      ofstream iddatafile(id_PATH, ios::out | ios::in | ios::app);
      // Append to file while the line counter is less than the difference between the number of lines and number of hadrons (iterate until we hit the accepted chain)
      ...
      // Write out id for accepted and rejected chains, separating the accepted and rejected chains by the string '&'
      iddatafile << "&" << endl;
      // Append the rejected fragmentations
      while (getline(idfile, line)) {
        iddatafile << line << endl;
    ...
    idfile.close(); // Close id_i.txt

    // Clear the temporary accept-reject file
    ofstream cfragfileEnd("pgun_data/acceptReject_i.txt", ios::out | ios::trunc);
    cfragfileEnd.close();

    // Clear the temporary mT2 file
    ofstream cMT2fileEnd("pgun_data/mT2_i.txt", ios::out | ios::trunc);
    cMT2fileEnd.close();

    // Clear the temporary fPrel file
    ofstream cfPrelfileEnd("pgun_data/fPrel_i.txt", ios::out | ios::trunc);
    cfPrelfileEnd.close();

    // Clear the temporary id file
    ofstream cidfileEnd("pgun_data/id_i.txt", ios::out | ios::trunc);
    cidfileEnd.close();
