"""
# RSA_data_converter.py is a part of the RSA package.
...
The program takes in the following arguments:
    --data_path_accept_reject: Path to the accept-reject .txt file
    ...
    --print_details: Print progress statements

An example command to run this program from the terminal is:

python RSA_nD_data_converter.py --data_path_accept_reject pgun_qqbar_finalTwo_accept_reject_z_a_0.72_b_0.88_sigma_0.335_N_1e4.txt 
--data_path_fPrel pgun_qqbar_finalTwo_fPrel_a_0.72_b_0.88_sigma_0.335_N_1e4.txt 
--data_path_mT2 pgun_qqbar_finalTwo_mT2_a_0.72_b_0.88_sigma_0.335_N_1e4.txt 
--data_path_hadrons pgun_qqbar_finalTwo_hadrons_a_0.72_b_0.88_sigma_0.335_N_1e4.txt
--data_path_id pgun_qqbar_finalTwo_id_a_0.72_b_0.88_sigma_0.335_N_1e4.txt
--write_path_id_mT2_accept_reject pgun_qqbar_mT2_accept_reject_a_0.72_b_0.88_sigma_0.335_N_1e4.npy  
--write_path_fPrel pgun_qqbar_fPrel_a_0.72_b_0.88_sigma_0.335_N_1e4.npy 
--write_path_hadrons pgun_qqbar_hadrons_a_0.72_b_0.88_sigma_0.335_N_1e4.npy
--print_details True
"""
...
#             arz_i = np.insert(arz_i, 0, int(lines_id[i].split()[0]), axis = 1)
#             # Iterate the counter
#             counter += 1
#         elif lines_ar[i] != '&\n':
...
#             arz_I = np.insert(arz_I, 0, int(lines_id[i].split()[1]), axis = 1)
#             arz_I = np.insert(arz_I, 0, int(lines_id[i].split()[0]), axis = 1)
#             # Append to the accept-reject array
...
#         if npad_event - len(arz_i) < 0:
#             print('\nToo many hadrons in event, change padding size to: ', len(arz_i))
#             os.remove(args.write_path_id_mT2_accept_reject)
#             sys.exit(1)
#         arz_i = np.pad(arz_i, ((0, npad_event - len(arz_i)),(0,0)))
#         # print(np.shape(arz_i))
#         # Write to .npy file located at write_path
#         with NpyAppendArray(args.write_path_id_mT2_accept_reject) as datafile:
#             datafile.append(np.array([arz_i]))
#         counter = 0
#         event_counter += 1

# # Print the dataset size
# pgun_accept_reject = np.load(args.write_path_id_mT2_accept_reject, mmap_mode = 'r')
